...
# class BybitSesh(CryptoSesh):
    ...
    #     position_statur = (await self.get_position(symbol))["size"]
...
class BybitSesh(CryptoSesh):
    ...
    def _format_order(self, order: dict) -> dict:
        ...
        type_str = (
            "Limit" if order_type == "limit" else
            "Market" if order_type == "market" else
            ("Stop" if order.get("triggerPrice") else "Market")
        )
        status_map = {
            "New": "Placed",
            "Created": "Placed",
        ...
        created_ms = order.get("createdTime") or order.get("updatedTime") or 0
        try:
        ...
        return {
            ...
            "side": side,
            "type": type_str,
            "volume": float(order.get("qty") or order.get("size") or 0),
            "price": float(order.get("price") or order.get("avgPrice") or 0),
            "sl": float(order.get("stopLoss") or 0),
            "tp": float(order.get("takeProfit") or 0),
            "time": int(created_ms // 1000),
            "status": status,
            "magic": 0,
            "comment": order.get("orderLinkId") or "",
        }
    ...
    async def get_instruments_info(self, symbol) -> dict:
        """
            ...
            Returns:
                dict: Información detallada del instrumento (formato estandarizado)
        """ 
        ...
        # Intentar extraer precisión de precio
        try:
            s = str(item['lotSizeFilter']['qtyStep'])
            digits = len(s.split(".")[-1]) if "." in s else 1
        except Exception:
            digits = 0
        ...
        return {
            ...
            "digits": digits,
            "tradeContractVolume": 1,
            "tradeTickVolume": 1,
            "tradeTickValue": 1,
            "volumeMin": float(item['lotSizeFilter']['minOrderQty']),
            "volumeMax": float(item['lotSizeFilter']['maxMktOrderQty']),
            "volumeStep": float(item['lotSizeFilter']['qtyStep']),
            "currencyBase": item['baseCoin'],
        }

    async def get_account_info(self, account_type: str = None):
        """
            Obtiene información de la cuenta de trading (formato estandarizado).
        """
        account_type = account_type if account_type else self._account_type
        ...
        return {
            ...
            "leverage": None,
            "balance": float(bal["walletBalance"]),
            "equity": float(bal["equity"]),
            "margin": None,
            "marginFree": (float(bal["walletBalance"]) - float(bal["totalPositionIM"])),
            "marginLevel": 0.0,
            "pnl": float(bal["cumRealisedPnl"]), 
    ...
    async def get_position(self, symbol) -> dict:
        """
            Obtiene el estado de la posición actual para un par de trading.
            Returns:
                dict: {"volume","side","price","pnl"} (formato estandarizado)
        """
        response = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: self._session.get_positions(category=self._category, symbol=symbol)
        )
        lst = response.get("result", {}).get("list") or []
        if not lst:
            return {"volume": 0.0, "side": "None", "price": 0.0, "pnl": 0.0}
        p = lst[0]
        return {
            "volume": float(p.get("size") or 0),
    ...
    # TODO: NEW agregar a las instrucciones de creacion de sesion
    async def get_new_orders(self, symbol) -> list:
        # Formatea órdenes y filtra por estado 'Placed' o 'Untriggered'
        orders = await self.get_all_orders(symbol)
        return [o for o in orders if o["status"] in ["Placed", "Untriggered"]]

    async def get_order_history(self, symbol) -> list:
        # Historial formateado (Filled/Cancelled/Rejected)
        orders = await self.get_all_orders(symbol)
        return [o for o in orders if o["status"] in ["Filled", "Cancelled", "Rejected"]]

    async def get_all_orders(self, symbol) -> list:
        """
            Obtiene el historial de órdenes estandarizado para un símbolo.
        """
        open_not_filled_task = asyncio.get_event_loop().run_in_executor(
            None,
            lambda: self._session.get_open_orders(
                category=self._category,
                symbol=symbol,
        ...
        open_filled_task = asyncio.get_event_loop().run_in_executor(
            None,
            lambda: self._session.get_open_orders(
                category=self._category,
                symbol=symbol,
        ...
        open_orders_not_filled, open_orders_filled = await asyncio.gather(
            open_not_filled_task, open_filled_task
        )

        formatted = [self._format_order(o) for o in (open_orders_not_filled + open_orders_filled)]
    ...
    async def get_order(self, symbol, order_id, max_attempts=15, wait_time=0.1) -> dict:
        all_orders = await self.get_all_orders(symbol)
        if all_orders:
            matching_orders = [d for d in all_orders if d["orderId"] == str(order_id)]
            if matching_orders:
    ...
    async def get_closed_pnl(self, symbol: str, from_date: int, to_date: int) -> list:
        ...
        for it in list_result or []:
            ts = it.get("updatedTime") or it.get("createdTime") or 0
            try:
                ts = int(ts)
            except Exception:
                ts = 0
            out.append({
                ...
                "pnl": float(it.get("closedPnl") or it.get("pnl") or 0),
                "volume": float(it.get("qty") or it.get("size") or 0),
                "price": float(it.get("avgExitPrice") or it.get("avgPrice") or 0),
                "time": int(ts // 1000),
                "commission": float(it.get("fees") or it.get("fee") or 0),
    ...
    async def close_position(self, symbol) -> (dict | None):
        position_info = await self.get_position(symbol=symbol)
        current_size = position_info["volume"]
        current_side = position_info["side"]
        ...
        if current_size > 0:
            if current_side == "Buy":
                ...
                return {"orderId": str(resp["result"]["orderId"])}
    ...
    # async def set_trading_stop(self, 
        ...
        #         tpLimitPrice=str(tp_price) if tp_price else None,
        #         slLimitPrice=str(sl_price) if sl_price else None,
        #         positionIdx=0,
        #     )
        # )
        # return {"success": (resp.get("retCode") == 0), "retCode": resp.get("retCode")}
    # async def set_trading_stop(self, 
        #                 symbol: str,
        #                 tp_price: float = None,
        #                 sl_price: float = None,
        ...
        # if current_volume <= 0 or current_side not in ("Buy", "Sell"):
        #     return []

        # opposite_side = "Sell" if current_side == "Buy" else "Buy"

        # raw_pos = await asyncio.get_event_loop().run_in_executor(
        #     None,
        #     lambda: self._session.get_positions(category=self._category, symbol=symbol)
        # )
        # pos_list = (raw_pos.get("result") or {}).get("list") or []
        # position_idx = pos_list[0].get("positionIdx", 0) if pos_list else 0

        # results = []

        # # Orden de Take Profit (Limit)
        # if tp_price is not None:
        #     tp_qty = tp_volume if tp_volume else current_volume
        #     tp_resp = await asyncio.get_event_loop().run_in_executor(
        #         None,
        #         lambda: self._session.place_order(
        #             category=self._category,
        #             symbol=symbol,
        ...
        #         "side": opposite_side,
        #         "type": "Limit",
        #         "orderId": str(tp_resp["result"]["orderId"]),
        #         "comment": "PartialTakeProfit" if tp_qty < current_volume else "FullTakeProfit",
        #         "success": tp_resp.get("retCode") == 0,
        #         "retCode": tp_resp.get("retCode"),
        #     })

        # # Orden de Stop Loss (Stop/condicionada)
        # if sl_price is not None:
        #     sl_qty = sl_volume if sl_volume else current_volume
        #     trigger_direction = 2 if current_side == "Buy" else 1  # caída para largos, subida para cortos
        #     sl_resp = await asyncio.get_event_loop().run_in_executor(
        #         None,
        #         lambda: self._session.place_order(
        #             category=self._category,
        #             symbol=symbol,
        ...
        #         "side": opposite_side,
        #         "type": "Stop",
        #         "orderId": str(sl_resp["result"]["orderId"]),
        #         "comment": "PartialStopLoss" if sl_qty < current_volume else "FullStopLoss",
        #         "success": sl_resp.get("retCode") == 0,
        #         "retCode": sl_resp.get("retCode"),
        #     })
    ...
    async def set_trading_stop(self, 
        ...
        if sl_price is not None:
            ...
            # Precio actual (MarkPrice) para validar/derivar triggerDirection
            last_price = await self.get_last_price(symbol)

            # Derivar triggerDirection segun relación sl_price vs last_price
            if current_side == "Buy":
                # En largo, SL usualmente bajo del precio actual (caída)
                trigger_direction = 2 if float(sl_price) < float(last_price) else 1
            else:  # current_side == "Sell"
                # En corto, SL usualmente arriba del precio actual (subida)
                trigger_direction = 1 if float(sl_price) > float(last_price) else 2
    ...
    async def cancel_order(self, symbol, order_id):
        ...
        return {"success": (resp.get("retCode") == 0), "retCode": resp.get("retCode"), "orderId": str(order_id)}
